...
pub fn render(
    ...
    if !meta.authors.is_empty() {
        let joined = meta
            .authors
            .iter()
            .map(|a| yaml_inline_string(a))
            .collect::<Vec<_>>()
            .join(", ");
        s.push_str(&format!("authors: [{joined}]\n"));
    }
    if let Some(p) = &meta.publisher {
        write_kv(&mut s, "publisher", p);
    }
    if let Some(p) = &meta.published {
        write_kv(&mut s, "published", p);
    }
    if let Some(p) = &meta.isbn {
        write_kv(&mut s, "isbn", p);
    }
    if let Some(p) = &meta.language {
        write_kv(&mut s, "language", p);
    }
...
fn pad_number(n: u64, width: usize, chapter: &str) -> Result<String, crate::Error> {
    let s = n.to_string();
    if s.len() > width {
        return Err(crate::Error::OffsetOverflow {
...
mod tests {
    ...
    fn padded_byte_field_has_constant_width() {
        let m = meta();
        let c1 = FrontmatterChapter {
            title: "Intro",
            offset: ChapterOffset { byte: 0, line: 1 },
        };
        let c2 = FrontmatterChapter {
            title: "Two",
    ...
    fn padded_value_parses_as_int() {
        let m = meta();
        let c = FrontmatterChapter {
            title: "X",
            offset: ChapterOffset { byte: 42, line: 7 },
        };
        let s = render(&m, &[c]).unwrap();
        let byte_line = s
            .lines()
            .find(|l| l.trim_start().starts_with("byte:"))
            .unwrap();
